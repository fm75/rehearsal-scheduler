...
class SchedulingDataLoader:
    ...
    def load_rds(self) -> pd.DataFrame:
        """
        Load rehearsal director information.
        
        Returns:
            DataFrame with RD details
        """
        cllick.echo("📊 Loading rds ...")
        scheduling_id = self.workbooks['scheduling']
        sheet_name = self.sheets['scheduling'].get('rd_constraints', 'rd_constraints')
        print(f"from {scheduling_id} tab {sheet_name}")
        return self._read_sheet_to_df(scheduling_id, sheet_name)
